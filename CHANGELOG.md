...
- **`__apcore_module_preview` disclosed the resolved binary and argv to a caller the ACL had denied.** `Executor::validate` reports the ACL verdict as a failed check and then still runs the module-level preflight and preview steps, so the envelope came back with `predicted_changes` naming the binary and the full argv, plus `requires_approval: true`, for a module the caller was not allowed to reach. The handler now short-circuits: on a failed `acl` check it returns `valid: false`, `requires_approval: false`, empty `predicted_changes`, and drops the `module_preflight` / `module_preview` checks, keeping the failed ACL check so the caller still learns why. No SDK short-circuited this before — Python's `_handle_preview_tool` and TypeScript's `PREVIEW` branch both serialise the whole preflight — so this is a Rust-first change rather than a port.
...
- **`explorer::ToolInfo` gained a public `annotations` field.** Callers constructing it with a struct literal must add `annotations: None` (or the tool's annotations); `serde` deserialisation of an older payload is unaffected, the field defaults to `None`.
