...
ever *called* from this crate's CLI or builder, and the ACL tier-2 diagnostic (FR-ACL-004) had never
been attempted here at all. This release closes those gaps. 1026 lib + 75 integration tests pass
across 17 integration binaries, 1101 total (was 1017 lib + 47 integration) — the integration count
grew because this release's own new coverage (`tests/openapi_backend_wiring.rs`, 6 tests; plus the
CLI/ACL/config test additions listed below) landed as integration tests, not because anything
regressed.

### Fixed

- **`openapi_backend` was never invoked from any entry point in this crate — not the CLI, not
  `APCoreMCPBuilder::build()`, nowhere.** `src/cli.rs` had zero `openapi`-related flags at all
  before this release (confirmed by grep). The module's own `openapi_backend(document: &Value, ...)`
  additionally only ever accepted an already-parsed document, unlike Python's and TypeScript's
  single polymorphic `spec` parameter — so even a caller holding a spec URL string had no function
  to call. Added:
  - `openapi_backend_from_spec(spec: &str, ...)` — resolves the location and fetches/parses via
    `apcore_toolkit::load_spec` (which already handles both the URL and local-path branches
    internally, plus JSON/YAML parsing), then delegates to `openapi_backend`.
  - `build_openapi_backend_from_config(openapi_config: &Value, ...)` — translates a raw `mcp.openapi`
    Config Bus mapping into `OpenAPIBackendOptions`, mirroring `acl_builder::build_acl_from_config`.
  - `CliArgs::extensions_dir` changed from `PathBuf` (required) to `Option<PathBuf>`; seven new
    flags (`--from-openapi`, `--openapi-base-url`, `--openapi-prefix`, `--openapi-include`,
    `--openapi-exclude`, `--openapi-header` (repeatable `Vec<String>`), `--openapi-no-deprecated`);
    `validate_args` now enforces the backend-source rule (at least one of `--extensions-dir` /
    `--from-openapi` / `mcp.openapi` on the Config Bus; combining the first two requires
    `--openapi-prefix`; malformed `--openapi-header` rejected).

- **`mcp.openapi.spec` set on the Config Bus alone now starts a server (PRD F-054 Acceptance
  Criterion 1) — via a new async builder method, because the synchronous one structurally cannot.**
  `mcp_defaults()` published an `openapi` key that nothing ever read back. Unlike `mcp.acl` /
  `mcp.middleware` (pure, synchronous, in-memory parsing), resolving `mcp.openapi.spec` means
  fetching and parsing a document, and `apcore-toolkit-rust`'s `load_spec` is built on async
  `reqwest` with no synchronous fallback — unlike Python, where `httpx.get` blocks synchronously by
  design and a plain `__init__` can resolve this inline. `APCoreMCPBuilder::build()` stays
  synchronous and unchanged; new `APCoreMCPBuilder::build_async()` resolves `mcp.openapi.spec` from
  the Config Bus when no backend was explicitly given, then delegates to `build()`. The CLI now
  calls `build_async()` unconditionally. A caller constructing `APCoreMCP` directly and relying
  purely on `apcore.yaml`'s `mcp.openapi.spec` (no CLI, no explicit backend) must call
  `build_async()` — calling the plain `build()` in that situation still returns
  `APCoreMCPError::BackendResolution`, exactly as it always has.

- **FR-ACL-004 (the §6.2.1 tier-2 "rules that protect nothing" startup diagnostic) had never been
  implemented in this crate at all.** Zero hits for `validate_rules`/`never_matches`/
  `protect_nothing` anywhere in `src/`, `tests/`, or this CHANGELOG before this release — not dead
  code (TypeScript's state before its own fix), simply never attempted, and never disclosed as
  missing despite the rest of the §6.2.1 work being covered in detail in 0.20.0's entry above.
  `apcore::ACL::validate_rules()` was available and unused. New
  `APCoreMCP::warn_acl_rules_that_protect_nothing`, called from `build_server_components` next to
  the existing `warn_if_control_surface_unprotected`, reading `Executor::acl`'s real
  `RuleValidationFinding` shape (`rule_index`/`condition_path`/`condition_key`/`effect` — there is
  no free-text reason field; the message is constructed here, matching Python and TypeScript's
  wording exactly after their own field-name fixes below).

- **`mcp_defaults()` never got an `openapi` key**, and the crate's own
  `test_mcp_defaults_key_set_matches_peer_sdks` parity test had a stale hardcoded expected list that
  didn't include it either — so the test was asserting parity with Python and TypeScript against an
  expectation that predated even Python's `openapi` key, and stayed green throughout. Both fixed;
  TypeScript's `MCP_DEFAULTS` also gained the same key in the same pass (see that repo's CHANGELOG).
...
- **`mcp.openapi.acknowledge_unapproved_writes` was documented as the suppression for the "nothing
  ...
  ever read it.** New `OpenAPIBackendOptions::acknowledge_unapproved_writes: bool` field, gating the
  `warn_if_writes_have_no_approval_path` call; `build_openapi_backend_from_config` reads it from the
  Config Bus mapping.

### Tests

- `tests/openapi_backend_wiring.rs` (6 cases, new file) — `openapi_backend_from_spec` and
  `build_openapi_backend_from_config` against real local (no network) OpenAPI documents, plus
  `build_async`'s no-backend-no-Config-Bus error path.
- `src/apcore_mcp.rs` — 2 new cases for the tier-2 diagnostic, using a `tracing_subscriber`
  `MakeWriter` capture buffer (no existing infrastructure in this crate asserted on `tracing::warn!`
  output before this — `warn_if_control_surface_unprotected`'s own content was likewise never
  directly asserted). Confirms the real field names and that `'?'` — the exact prior-bug string —
  never appears.
- `src/acl_builder.rs` — 2 new cases for the success log, same capture technique.
- `src/cli.rs` — 7 new `validate_args` / `CliArgs` cases for the relaxed backend-source rule.
- `tests/cli_integration.rs` — `missing_extensions_dir_exits_nonzero` renamed and rewritten:
  clap no longer rejects an empty invocation (exit 2); `validate_args` does (exit 1), with the
  backend-source message asserted on stderr.
...
- `docs/features/openapi-backend.md`'s `## Contract: openapi_backend` Returns section and
  ...
  universal. Measured directly: only this crate's writer (`apcore-toolkit-rust`'s
  `HTTPProxyRegistryWriter`, which builds a full `ModuleDescriptor`) preserves it; Python's and
  TypeScript's both drop it — an upstream (apcore-toolkit) inconsistency, now stated as Rust-only
  in both documents.
...
PROTOCOL_SPEC §6.2.1 pattern-array closure. Raises `apcore` to `">=0.30"` and `apcore-toolkit` to
`">=0.11.1"`. 1017 lib + 47 integration tests pass.

**This release absorbs a compile break.** apcore 0.29.0 made `ACLRule` `#[non_exhaustive]`, so every
`ACLRule { .. }` struct literal in this crate stopped compiling — five of them, all but one in
tests. See *Fixed* below.

### Added

- **`src/openapi_backend.rs` — a third backend source, and the first with full parity.**
  `openapi_backend(document, registry, options)` composes apcore-toolkit's shipped pieces
  (`OpenAPIScanner::scan` → `HTTPProxyRegistryWriter::write`) into a populated `Registry`: no
  scanning logic, no schema conversion and no new execution path. Directory auto-discovery is
  Python/TypeScript-only because apcore's public Rust API has no runtime directory discovery — an
  OpenAPI document needs no discoverer, so this is the first backend source that works here without
  the caveat qualifying every `--extensions-dir` sentence in this crate's documentation.
...
- **A module-ID projection, without which the backend serves nothing.** apcore-toolkit's
  ...
  set. `project_module_id` lowercases and maps `-` to `_` (`is_legal_segment` checks the per-segment
  pattern without pulling in a regex dependency); a segment that still does not begin with a
  lowercase letter (`/v1/2fa`) is **skipped with a `tracing::warn!`** rather than repaired, because
  completing it means inventing a character. The projection runs after any caller-supplied
  `transform_module` and before the scanner's `deduplicate_ids`.
...
- **A startup warning that nothing will ask for approval before a write.** Reports the absence of an
  ...
  the rule apcore states on `GovernanceState::unprotected_control_surface`.

- **`resolve_spec_location`** for the path-typed `mcp.openapi.spec` value: an `http(s)://` value
  verbatim, a set-but-empty value discarded with a warning so resolution falls through, and a
  relative path resolved against `Config::project_root` lexically (`.`/`..` normalized without
  touching the filesystem, so the result is comparable across the three SDKs for a path that need
  not exist). apcore 0.30.0's own §9.2.1 protections do not reach a consumer namespace:
  `Config::path_typed_keys()` is a fixed set of apcore's own keys and never consults a namespace
  registered through `Config::register_namespace`.
...
- **`build_acl_from_config` validates rule by rule.** `ACL::try_new` validates the whole list and
  apcore exports no per-rule validator, so a throwaway single-rule `try_new` is the only way to
  learn which rule was at fault while the builder still knows its real index. Its refusal is
  re-raised as `APCoreMCPError::Config` prefixed `mcp.acl.rules[i]`, apcore's message preserved
  verbatim after the prefix.

- **Required floors: `apcore = ">=0.30"`, `apcore-toolkit = ">=0.11.1"`.** apcore **0.29.0** is the
  correctness floor; apcore-toolkit **0.11.0** the capability floor — and also where this crate's
  `HTTPProxyRegistryWriter` stopped rejecting `HEAD`/`OPTIONS`/`TRACE` before any network call, a
  defect that would have made the Rust OpenAPI path quietly broken for those three methods.
  **0.11.1** changes no API and forces apcore **0.30.0**, independently needed for
  `Config::project_root`.

### Fixed

- **`ACLRule` became `#[non_exhaustive]` in apcore 0.29.0 — five struct literals stopped
  compiling.** All are rewritten to `ACLRule::new(callers, targets, effect)` plus field assignment,
  the form apcore's own api-surface-conventions §9.3 names as the one that compiles downstream. Note
  the comment two of them carried — *"`ACLRule` is not `#[non_exhaustive]`, so every literal now
  names this field"*, added in 0.19.0 — was true until 0.29.0 and is now corrected in place.

### Tests

- `tests/openapi_backend_conformance.rs` drives the new shared fixture `openapi_backend.json`
  (module cases, spec-resolution cases, error cases), with `tests/common::register_stub` seeding the
  target registry so the collision preflight is reachable.
- `tests/acl_conformance.rs` drives `acl_config.json` at `contract_version` 1.2 and understands the
  1.2 additions `expected_error_substrings`, `expected_error_names_field` and `must_not_contain`.
  The field assertion is on the **bare** name: apcore-python and apcore-js write `'callers'` while
  apcore-rust writes `'callers[1]'`, naming the offending element, and the bare token is the only
  spelling all three share. This driver also caught a fixture case that defeated its own
  `default_effect` probe — `["$not", "banned.*"]` matches almost every module ID, so a `*` caller
  fired the rule on the probe; the shared case now names a literal caller.
...
- **`apcore = "0.27"` could never resolve 0.28.0 (dependency bug).** A bare version string is cargo's caret syntax, so `"0.27"` meant `>=0.27.0, <0.28.0` — `cargo update` had no way to pick up 0.28.0 no matter how long it ran. Raised to `"0.28"`, the same bounded-per-minor-release convention the 0.27 bump already established (CHANGELOG "[0.18.0]"): a deliberate, re-vetted pin, not an unbounded range. `apcore-toolkit`'s floor is raised to `">=0.10.2"` (was `">=0.10.0"`) to record that this line has been vetted against that patch specifically; apcore-toolkit 0.10.1→0.10.2 made no code or API changes (bumped its own `apcore` floor only — see apcore-toolkit's CHANGELOG), so `src/markdown.rs` needed no changes.
- Two test-only `ACLRule { .. }` struct literals (`apcore_mcp.rs`, `server/async_task_bridge.rs`) gained the `approval: None` field apcore 0.28.0 added to `ACLRule` (apcore#108); `ACLRule` is not `#[non_exhaustive]`, so these were compile breaks under the corrected dependency, not runtime bugs.
- **`system.usage.module`'s resource template was missing the `{?period}` query-expansion suffix.** `SYSTEM_RESOURCE_TEMPLATE_MODULES`'s literal `uri_template` for this module was `apcore://system.usage.module/{module_id}` — indistinguishable from the two templates that take no query parameter — instead of `apcore://system.usage.module/{module_id}{?period}`, the shape aiperceivable/apcore-mcp#15's URI-convention table specifies and apcore-mcp-typescript's `systemResourceUriTemplate()` already emitted. Caught by the new cross-language `system_surface.json` conformance fixture below: TypeScript had it right, Python and Rust had silently drifted. `period` was already parsed and forwarded correctly at read time (`read_system_resource`) — only the *declared* template string was wrong.

### Added

- **`mcp.acl` accepts the apcore 0.28.0 `approval` rule key (argument-scoped approval, apcore#108).** A rule can now carry `approval: "required"` alongside `effect: allow`, gating the call on a human decision even though the ACL itself allows it. Both spellings apcore's own `ApprovalRequirement` deserializes are accepted — `"required"` and `"not_required"`. An earlier iteration of this change accepted only `"required"`, reasoning that `mcp.acl` should have exactly one spelling for "no approval requirement here"; that made this bridge **stricter than the schema it bridges**, so a rule that loads fine from apcore's own `acl/` directory failed at startup when the identical rule was carried through the Config Bus. Rejecting the redundant spelling prevented no misconfiguration (`ACLRule::approval_required()` treats `Some(NotRequired)` and `None` identically) while breaking a valid configuration. `build_acl_from_config` now calls `ACL::try_new` instead of the panicking `ACL::new`, so an invalid combination (`approval: required` on a `deny` rule) surfaces as a `Config` error — matching this function's documented "fail loudly via `Err`" contract — instead of unwinding the process. (aiperceivable/apcore-mcp#14, aiperceivable/apcore-mcp-rust#6)
- **`system.health.*`, `system.usage.*` and `system.manifest.*` are exposed as MCP resources instead of tools.** The three parameterless modules (`system.health.summary`, `system.usage.summary`, `system.manifest.full`) are static `apcore://` resources; the three that require a `module_id` (`system.health.module`, `system.usage.module`, `system.manifest.module`) are registered as `apcore://.../{module_id}` resource templates — a mechanism added to this crate's hand-rolled MCP layer (`resources/templates/list`) since none existed before. Every read dispatches through `ExecutionRouter::handle_call`, so ACL, approval, and redaction apply exactly as they do to a `tools/call`. `system.control.*` is unaffected and stays a tool. (aiperceivable/apcore-mcp#15(a), aiperceivable/apcore-mcp-rust#6)
- **Startup warning for an unprotected `system.control.*` surface.** `APCoreMCP::build_server_components` now calls the apcore 0.28.0 `Executor::governance_state()` accessor (apcore#97) after the executor is fully assembled and before any transport starts listening; when `unprotected_control_surface` is true it logs a loud, multi-line `tracing::error!` naming which of the four protections are missing (no ACL / ACL configured but its gate isn't wired / no approval handler and a non-strict policy / not every control module requires approval) with a remediation pointer. Warning only — never blocks startup. (aiperceivable/apcore-mcp#15(b), aiperceivable/apcore-mcp-rust#6)
- **`com.aiperceivable/management` initialize extension.** `MCPServerFactory::build_init_options` takes an optional `ManagementCapabilities { health, usage, manifest, control }`; when at least one is `true`, `initialize`'s `capabilities.extensions` advertises `{"com.aiperceivable/management": {"surfaces": [...], "protocolVersion": "1.30.0"}}`, naming only the surfaces actually present. `ServerCapabilities` gains an `extensions: Option<Value>` field (this crate has no MCP SDK to negotiate the shape with, so it carries a raw JSON value). Purely additive: a client that never declares — or even looks at — this extension gets identical tool/resource access. (aiperceivable/apcore-mcp#16, aiperceivable/apcore-mcp-rust#6)
- `acl_builder`'s module-doc ACL template now uses the correct `system.*` namespace (it previously showed `sys.*` / `sys.reload` / `sys.toggle`, which do not exist) and is the full three-rule reference policy for gating the management surface, including the `@external` / `conditions` mechanics MCP callers require. A new test asserts every `targets` pattern in that template matches at least one module id `apcore::sys_modules::register_sys_modules` actually registers. `config::get_acl_config`'s doc comment now states plainly that enabling `sys_modules.enabled` without configuring `mcp.acl` leaves the whole management surface unauthenticated. (aiperceivable/apcore-mcp#14, aiperceivable/apcore-mcp-rust#6)

### Tests

- New `tests/system_surface_conformance.rs`: drives the shared `apcore-mcp/conformance/fixtures/system_surface.json` fixture against a registry built by a real `register_sys_modules()` call, asserting `build_tools()` / `register_resource_handlers()` output matches byte-for-byte — the Python and TypeScript bridges run the identical fixture, operationalizing aiperceivable/apcore-mcp#15's cross-language parity acceptance criterion as a regression test instead of a one-time manual check.
- **New end-to-end approval-gating tests** (`apcore_mcp.rs`, `test_acl_approval_rule_gates_a_matching_tools_call` / `..._does_not_gate_a_non_matching_tools_call`). Every other `approval` test in this crate stopped at the Config Bus parsing layer — they proved the key was *accepted*, not that it *did* anything, so the claim above ("gating the call on a human decision even though the ACL itself allows it") was unverified at the MCP boundary. These drive a real `ACL` + real approval handler + real module through `ExecutionRouter::handle_call` with the module's own `requires_approval` annotation set to **false**, so the ACL rule is the only possible source of the requirement, and an argument-scoped condition (`conditions.arguments.has_key`) decides it: a call carrying `recursive` reaches the approval handler, an otherwise identical call without it does not.
- The shared `acl_config.json` fixture gained `approval` contract cases (see the spec repo's 0.19.0 entry), which is what pins the accepted-value set across the three bridges rather than leaving each to decide independently — the gap that allowed the `not_required` divergence above.

## [0.18.1] - 2026-08-20

Patch release. Bumps the required `mcp-embedded-ui` floor to `"0.5"` (was `"0.4"`). No `apcore-mcp` code changes — `mcp_embedded_ui::create_mount` is called exactly as before. All 1056 tests pass unmodified against mcp-embedded-ui 0.5.0.

### Changed

- **Required `mcp-embedded-ui` floor raised to `"0.5"`.** A consumer who mounts the Explorer inherits mcp-embedded-ui 0.5.0's Try-It editor prefill change (spec F6/FR-1): the prefill now emits only the keys listed in `inputSchema.required`, using each property's declared `default` when present and `null` otherwise, instead of inventing a type-based value (`""`, `0`, ...) for every property.
...
- **Interactive approval works from a handler the router did not construct (apexe#29).** `--enable-approval` denied every gated call on a CLI-launched server: `ElicitationApprovalHandler` had no way to reach the connected client, so the human-in-the-loop path the docs describe did not exist. Two independent breaks stood in the way.
  ...
  **The transport now supplies the session.** The two breaks above closed the library half; `apcore-mcp serve` still could not prompt, because nothing produced a `SessionHandle`. The MCP `call_tool` handler goes through `handle_call`, whose `extract_extra` returned `session: None` unconditionally — a plain JSON `extra` cannot carry a trait object — and no production code implemented the trait.

  The new `server::session` module adds the server-to-client half the transports never had: `OutboundSink` (how one connection writes a server-initiated message), `McpSession` (one connection's request/response correlation, implementing `SessionHandle`), and `SessionRegistry` (transport-minted session id to live connection). `extract_extra` now resolves the session id the transport stamps onto `params._meta` against that registry, which is how the trait object survives the hop through JSON. stdio and SSE both register their connections and deregister them by RAII on disconnect, error, and unwind alike.

  `initialize` records each client's advertised capabilities against its own connection, and a client that never advertised `elicitation` is refused before anything is written — failing closed rather than waiting out a timeout for an answer no conforming client owes. Server-initiated requests carry a uuid-prefixed id; the client's answer is intercepted before the handler, which would otherwise reject a message with no `method` as `-32600`. A pending entry is cleared by RAII on all four exit paths: answer, timeout, failed write, and disconnect. Requests time out after 5 minutes so a silent client cannot pin a call forever.

  `apexe` can therefore move `--enable-approval` off `DenyApprovalHandler` once it takes a release carrying this.
...
- **BREAKING (behaviour): stdio dispatches messages concurrently.** The read loop used to await each `handle_message` inline, so nothing was read while a call ran. For elicitation that is not merely slow but fatal: a call that asks the client a question blocks until the answer arrives, and the answer arrives on stdin — which the loop is not reading, because it is blocked inside the very call that wants to send the question. Handlers now run in a `FuturesUnordered` polled alongside the reader and the outbound queue, matching what SSE already does; all three share one task, so the writer stays exclusively owned and neither `R` nor `W` gained a bound. **Responses may now arrive in an order other than the one the client sent** — JSON-RPC correlates by `id`, and a client that assumed FIFO must not. EOF now drains work already in flight rather than returning immediately, and fails any outstanding elicitation instead of holding the drain open for its timeout. Lines are read through `tokio_stream`'s `LinesStream` because `read_line` is not cancellation-safe inside a `select!` and would lose a partially read line whenever another arm won the race.
- **Elicitation is deliberately unavailable on the streamable-HTTP transport.** That transport mints one session id per *application*, not per connection, so registering it would point every client at a single session: a prompt raised by one client's tool call could be written to another client's stream, and any client could answer another's prompt — the cross-connection leak the SSE isolation work above removed. A gated call there is denied rather than prompted, and a test pins both the denial and the empty registry so the wiring is not added later for the sake of symmetry. Per-connection sessions have to land first.
