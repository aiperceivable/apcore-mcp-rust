...
- **`mcp.acl` accepts the apcore 0.28.0 `approval` rule key (argument-scoped approval, apcore#108).** A rule can now carry `approval: "required"` (the only accepted spelling) alongside `effect: allow`, gating the call on a human decision even though the ACL itself allows it. `build_acl_from_config` now calls `ACL::try_new` instead of the panicking `ACL::new`, so an invalid combination (`approval: required` on a `deny` rule) surfaces as a `Config` error — matching this function's documented "fail loudly via `Err`" contract — instead of unwinding the process. (aiperceivable/apcore-mcp#14, aiperceivable/apcore-mcp-rust#6)
